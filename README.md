...
详见 `AGENTS.md` 的「协议陷阱」（第 2、8、9、10、11 条）。

## 封面为什么慢得下来

封面全部来自用户自己的服务器（走它的 `/api/p_static` 代理），所以取图这件事按「别把自家小机器打爆」来做，而不是按 CDN 的思路：

| 手段 | 作用 |
|---|---|
| `LazyColumn` 只组合可见行 | 屏幕外的封面**根本不会发起请求** |
| 并发上限 4 | Coil 默认跑在 `Dispatchers.IO`，快速滑动会变成一次十几发的并发洪峰 |
| 磁盘缓存 64 MB + 内存缓存 | 同一个 URL **每次安装只取一次**，来回滚动与重开 App 都为 0 请求 |
| 尊重服务端缓存头 | 这些文件带长期 immutable 缓存头，可直接长留 |
| 只请求 48dp 缩略图 | 列表行不需要 1000px 原图，避免逐行解码大图 |
| 复用同一个 OkHttp 客户端 | 连接复用；超时集中在一处可查 |

结果是：**一个封面一次请求**，与列表长度无关。曲目封面在列表加载时**只做 URL 改写**（不取图），真正的请求等行滚进视野才发生。
